...
private:
    ...
    // v5: per-limb meshes; each LimbMesh produces both lateral halves (faces_right/_left).
    std::unique_ptr<Model> upper_right;
    std::unique_ptr<Model> upper_left;
    std::unique_ptr<Model> lower_right;
    std::unique_ptr<Model> lower_left;
    // Solid black grip block bridging the two inboard limb tips when a rigid
    // handle is configured. Null for a flexible / zero-length handle.
    std::unique_ptr<Model> handle;
};
